...
function augment (out, post, configuration) {
  if (!out.host) {
  ...
  if (configuration.tor && out.address) {
    return tor.exitNode(out.address, configuration.ip, 80, function(viaTor) {
...
function analyse (line, post, configuration) {
  var match, out = {};
  ...
  return augment(out, post, configuration);
}
  ...
  module.exports['cpu-timing'] = function (context, configuration) {
    ...
    if (duser > 0) {
      augment({
        ...
        'action': 'CPU',
        'label': 'UserTime',
        'value': duser }, context.post, configuration);
    }
    if (dnice > 0) {
      augment({
        'non-interactive': '1',
        ...
        'action': 'CPU',
        'label': 'NiceTime',
        'value': dnice }, context.post, configuration);
    }
    if (dsys > 0) {
      augment({
        'non-interactive': '1',
        ...
        'action': 'CPU',
        'label': 'SystemTime',
        'value': dsys }, context.post, configuration);
    }
    if (dirq > 0) {
      augment({
        'non-interactive': '1',
        ...
        'action': 'CPU',
        'label': 'IRQTime',
        'value': dirq }, context.post, configuration);
    }
          ...
          for (i in flat) {
            ...
            if (diff > configuration.threshold.bandwidth) {
              ...
              augment({
               ...
               'action': 'Network',
               'label': i,
               'value': diff }, context.post, configuration);
            }
          }
        }
      }
