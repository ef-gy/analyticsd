...
function analyse (line) {
  ...
  } else if (match = /(.+) ([^ ]+) sshd.([0-9]+).: .+ received disconnect from (.+): .+ unable to connect using the available authentication methods/i.exec(line)) {
    out = {
      'date': match[1],
      'host': match[2],
      'type': 'event',
  ...
  /* scanlogd events */
  } else if (match = /(.+) ([^ ]+) scanlogd: (.+) to .+/i.exec(line)) {
    out = {
      'date': match[1],
      'host': match[2],
      'type': 'event',
      'category': 'Service',
      'action': 'scanlogd',
      'label': 'PortScanDetected',
      'session': 0,
      'address': match[3],
      'page': 'virtual/scan:' + match[2]
    };
  }
